...
fun main() {
    val input: List<String> =
        ClassLoader.getSystemResource("inputDay7.txt")!!.readText().split(System.lineSeparator())
...
fun part2(input: List<String>) =
    input
        .map { it.split(" ") }.map { (text, bid) ->
            val cards =
                text.map { card -> values2.indexOf(card).let { if (it > -1) it + 10 else card.digitToIntOrNull() ?: 1 } }
            val groups = (2..13)
                .map { swap ->
